...
public class AppExceptionHandler {
    ...
    public ResponseEntity<ExceptionResponse> handleEntityNotFoundException(EntityNotFoundException e, HttpServletRequest request) {
        String localizedMessage = messageSource.getMessage(
                e.getMessage(), new Object[]{}, request.getLocale());
        ExceptionResponse response = new ExceptionResponse(e.getMessage(), e.getClass().getSimpleName(), buildErrorMessage(localizedMessage, e.getId()), request.getServletPath());
        return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(HttpMediaTypeNotSupportedException.class)
    public ResponseEntity<ExceptionResponse> handleMediaTypeException(HttpMediaTypeNotSupportedException e, HttpServletRequest request) {
        String errorMessage = messageSource.getMessage(
                "50101", new Object[]{}, request.getLocale());
        ExceptionResponse response = new ExceptionResponse("50101", e.getClass().getSimpleName(),errorMessage, request.getServletPath());
        return new ResponseEntity<>(response, HttpStatus.UNSUPPORTED_MEDIA_TYPE);
    }

    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public ResponseEntity<ExceptionResponse> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e, WebRequest wr) {
        String errorMessage = messageSource.getMessage(
    ...
    public ResponseEntity<ExceptionResponse> handleHttpMessageNotReadableException(HttpMessageNotReadableException e, HttpServletRequest request) {
        ...
        ExceptionResponse response = new ExceptionResponse("50104",e.getClass().getSimpleName(),errorMessage, request.getServletPath());
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ExceptionResponse> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e,  HttpServletRequest request) {
        ...
        ExceptionResponse response = new ExceptionResponse("50105", e.getClass().getSimpleName(),errorMessage, request.getServletPath());
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }
